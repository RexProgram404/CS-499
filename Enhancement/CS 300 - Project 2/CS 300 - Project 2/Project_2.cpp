...
void loadCourses(string csvPath, HashTable *hashTable) {
	...
	while (getline(inputFile, line)) {
		...
		if (parts.size() < 2) {
			...
			parts.erase(parts.begin());
			if (parts.size() > 0) {
				course.prerequisite.insert(course.prerequisite.end(),
...
Course HashTable::Search(string courseId) {
	...
	if (node != nullptr && node->key != UINT_MAX
			&& node->course.courseID.compare(courseId) == 0) { //if entry is found
			//return node course
		return node->course;
	}
...
void Hashtable::setup(int argc, char *argv[], string &file, string &courseKey) {
	switch (argc) {
	case 2:
		file = argv[1];
		courseKey = "CSCI100";
...
void Hashtable::menu(string file, string courseKey, Course *course, Hashstable *courseTable) {
	int choice = 0;
	while (choice != 9) {
		cout << "Menu:" << endl;
...
int main(int argc, char *argv[]) {
	...
	setup(argc,crgv[],file,courseKey);

	HashTable *courseTable;
	Course course;
	courseTable = new HashTable();
	
	menu(file,courseKey,course,courseTable);
